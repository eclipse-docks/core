import{i as e,t}from"./chunk-DseTPa7n.js";import{_ as n,d as r,f as i,l as a,n as o}from"./fs-access-DTXPsV1W-Dp772n0E.js";import{F as s,P as c,T as l,W as u,_t as d,c as f,dt as p,ft as m,n as h,o as g,pt as _,q as v,ut as y,vt as b}from"./dist-DkbPbSj1.js";import"./lit-BXsh-fRu.js";var x=`dataview/publish`,S=`dataview/added`,C;function w(e){return e==null?``:String(e)}function T(e,t){return e.length===0?!1:e.every(e=>{let n=e[t];if(n==null)return!0;let r=Number(n);return Number.isFinite(r)})}function E(e,t,n){if(n){let n=Number(e),r=Number(t);return Number.isFinite(n)?Number.isFinite(r)?n-r:-1:+!!Number.isFinite(r)}return w(e).localeCompare(w(t),void 0,{numeric:!0})}var D=class extends _{static{C=this}constructor(...e){super(...e),this.data={columns:[],rows:[]},this.emptyMessage=`No data.`,this.sortColumnIndex=null,this.sortDirection=`asc`,this.filterQuery=``,this.pageSize=25,this.currentPage=0}static{this.PAGE_SIZE_OPTIONS=[10,25,50,100]}get columns(){return Array.isArray(this.data?.columns)?this.data.columns:[]}get rows(){return Array.isArray(this.data?.rows)?this.data.rows:[]}get filteredRows(){let e=this.filterQuery.trim().toLowerCase();return e?this.rows.filter(t=>t.some(t=>w(t).toLowerCase().includes(e))):this.rows}get sortedRows(){let e=this.filteredRows;if(this.sortColumnIndex==null||this.sortColumnIndex<0)return e;let t=this.sortColumnIndex,n=T(e,t),r=this.sortDirection===`asc`?1:-1;return[...e].sort((e,i)=>{let a=e[t],o=i[t];return r*E(a,o,n)})}get totalRows(){return this.sortedRows.length}get pageCount(){let e=this.totalRows;return e===0?1:Math.ceil(e/this.pageSize)}get pagedRows(){let e=this.sortedRows,t=this.clampedPage*this.pageSize;return e.slice(t,t+this.pageSize)}get clampedPage(){let e=this.pageCount;return e<=0?0:Math.min(this.currentPage,e-1)}goToPage(e){let t=Math.max(0,this.pageCount-1);this.currentPage=Math.max(0,Math.min(e,t)),this.requestUpdate()}onPageSizeChange(e){let t=e.target.value,n=parseInt(t,10);!Number.isFinite(n)||n<1||(this.pageSize=n,this.currentPage=0,this.requestUpdate())}onSort(e){this.sortColumnIndex===e?this.sortDirection=this.sortDirection===`asc`?`desc`:`asc`:(this.sortColumnIndex=e,this.sortDirection=`asc`),this.requestUpdate()}onFilterInput(e){this.filterQuery=e.target.value,this.requestUpdate()}clearFilter(){this.filterQuery=``,this.requestUpdate()}getSortAria(e){return this.sortColumnIndex===e?this.sortDirection===`asc`?`ascending`:`descending`:`none`}render(){let{columns:e}=this,t=this.totalRows,n=this.pagedRows,r=this.clampedPage,i=this.pageCount,a=t===0?0:r*this.pageSize+1,o=Math.min((r+1)*this.pageSize,t);return e.length===0&&t===0&&this.rows.length===0?d`<div class="table-empty">${this.emptyMessage}</div>`:d`
      <div class="table-toolbar">
        ...
        <div class="paging-controls">
          ...
          >
            ${C.PAGE_SIZE_OPTIONS.map(e=>d`<wa-option value=${String(e)}>${e}</wa-option>`)}
          </wa-select>
      ...
      <div class="table-wrap">
        <table class="result-table">
              ...
              ${e.map((e,t)=>d`
                  <th scope="col" role="columnheader" aria-sort=${this.getSortAria(t)}>
                    ...
                    >
                      ...
                      ${this.sortColumnIndex===t?d`<wa-icon
                            name=${this.sortDirection===`asc`?`arrow-up`:`arrow-down`}
                            label=${this.sortDirection}
                          ></wa-icon>`:d`<wa-icon name="arrows-up-down" label="Sort"></wa-icon>`}
                    </button>
                  </th>
                `)}
            </tr>
          ...
          <tbody>
            ${n.length===0?d`<tr><td colspan=${e.length} class="table-empty-cell">No matching rows.</td></tr>`:n.map(e=>d`
                    <tr>
                      ${e.map(e=>d`<td>${w(e)}</td>`)}
                    </tr>
                  `)}
...
`);return a?`${i}\n${a}`:i}async onExportCsv(){let e=this.displayed;if(!(!e||!this.hasData))try{let t=this.toCsv(e),n=e.title?.trim()||`dataview`,r=new Date().toISOString().replace(/[:.]/g,`-`),i=`${n.replace(/[^a-zA-Z0-9-_]+/g,`_`)}-${r}.csv`,a=await l(`directory`);if(!a)return;await this.executeCommand(`touch`,{path:`${a}/${i}`,contents:t})}catch(e){v(e instanceof Error?e.message:String(e))}}async doInitUI(){let e=await this.getDialogSetting();e&&typeof e.autoActivateTab==`boolean`&&(this.autoActivateTab=e.autoActivateTab),this.subscribe(S,async()=>{await this.refreshPersistedList(!0),this.autoActivateTab&&this.activateContainingTab()}),await this.refreshPersistedList(!1)}async refreshPersistedList(e){this.loadingList=!0,this.requestUpdate();try{if(this.persistedList=await M.listViews(),e&&this.persistedList.length>0){let e=this.persistedList[this.persistedList.length-1];this.selectedStorageKey=e.storageKey,this.selectedView=await M.getView(e.storageKey)}else this.selectedStorageKey?this.selectedView=await M.getView(this.selectedStorageKey):this.selectedView=null}catch(e){v(e instanceof Error?e.message:String(e)),this.persistedList=[],this.selectedView=null}finally{this.loadingList=!1,this.requestUpdate()}}async selectStorageKey(e){if(this.selectedStorageKey=e,!e){this.selectedView=null,this.requestUpdate();return}try{this.selectedView=await M.getView(e)}catch(e){v(e instanceof Error?e.message:String(e)),this.selectedView=null}this.requestUpdate()}async onAutoActivateChange(e){let t=e.target.checked;this.autoActivateTab=t;let n=await this.getDialogSetting()??{};await this.setDialogSetting({...n,autoActivateTab:t})}async onHistorySelect(e){let t=e.detail?.item?.value??``;!t||t===`__stats__`||await this.selectStorageKey(t)}async onDeleteView(e,t){e.stopPropagation(),e.preventDefault();try{await M.deleteView(t),this.selectedStorageKey===t&&(this.selectedStorageKey=``,this.selectedView=null),await this.refreshPersistedList(!0)}catch(e){v(e instanceof Error?e.message:String(e))}}async onClearHistory(){try{await M.clearAllViews(),this.selectedStorageKey=``,this.selectedView=null,await this.refreshPersistedList(!1)}catch(e){v(e instanceof Error?e.message:String(e))}}renderToolbar(){let e=this.selectedView??this.dataview,t=this.persistedList.find(e=>e.storageKey===this.selectedStorageKey),n=t?.title??e?.title??(this.persistedList.length>0?`Latest data view`:`No data`),r=t?.createdAt??e?.createdAt?new Date(t?.createdAt??e?.createdAt).toLocaleString():null,i=e?.source??null,a=i?`${n} · ${i}`:n,o=r?`${a} (${r})`:a;return d`
        ...
        >
          ...
          ${this.persistedList.map(e=>d`
              <wa-dropdown-item value=${e.storageKey}>
                ${e.source?`${e.title} · ${e.source}`:e.title}
                ${e.createdAt?d`<span style="opacity: 0.7; margin-left: 0.5rem; font-size: 0.75em;">
        ...
        ${s(e,()=>d`<wa-divider orientation="vertical"></wa-divider><span>${o}</span>`)}
    `}renderTable(e){return this.hasData?d`<docks-data-table .data=${e.data}></docks-data-table>`:d`<div class="result-empty">No data.</div>`}renderContent(){let e=this.displayed;return e==null?d`<div class="result-empty">No data.</div>`:this.renderTable(e)}static{this.styles=b`
    :host {
      display: flex;
      flex-direction: column;
      height: 100%;
  ...
  `}};c([p({attribute:!1})],N.prototype,`dataview`,void 0),c([y()],N.prototype,`persistedList`,void 0),c([y()],N.prototype,`selectedStorageKey`,void 0),c([y()],N.prototype,`selectedView`,void 0),c([y()],N.prototype,`loadingList`,void 0),c([y()],N.prototype,`autoActivateTab`,void 0),N=c([m(`docks-dataview`)],N);function P(e){let t=k.default.parse(e,{header:!0,skipEmptyLines:!0}),n=t.meta.fields??[];return{columns:n,rows:t.data.map(e=>n.map(t=>e[t]))}}M.init(),n.put(`dataviewerService`,M),u.registerContribution(f,{name:`view.dataviewer`,label:`Data Views`,icon:`table`,component:e=>d`<docks-dataview id="${e}"></docks-dataview>`}),h.registerEditorInputHandler({editorId:`system.dataviewer-table`,label:`Table`,icon:`table`,ranking:800,canHandle:e=>{if(!(e instanceof o))return!1;let t=e.getName().toLowerCase();return t.endsWith(`.csv`)||t.endsWith(`.tsv`)},handle:async e=>{e.getName();let{columns:t,rows:n}=P(await e.getContents()??``),r={columns:t,rows:n};return{title:e.getWorkspacePath(),data:r,key:e.getWorkspacePath(),icon:`table`,state:{},component:()=>d`<docks-data-table .data=${r}></docks-data-table>`}}});function F(){}export{F as default};