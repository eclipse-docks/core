import{d as e,m as t,n,o as r,r as i,t as a,u as o}from"./fs-access-DTXPsV1W-Dp772n0E.js";import{C as s,G as c,N as l,U as u,dt as d,gt as f,i as p,lt as m}from"./dist-1yslHcKF.js";var h=class{constructor(e){if(this.baseUrl=e.url,e.username!==void 0){let t=`${e.username}:${e.password??``}`;this.authHeader=`Basic ${btoa(t)}`}}async propfind(e,t=1){let n=await fetch(new URL(e,this.baseUrl).href,{method:`PROPFIND`,headers:this.buildHeaders({Depth:t.toString(),"Content-Type":`application/xml`}),body:`<?xml version="1.0" encoding="UTF-8"?>
            ...
            </d:propfind>`});if(!n.ok)throw Error(`WebDAV PROPFIND failed with status ${n.status}`);let r=await n.text();return this.parseMultiStatus(r)}async getFile(e){let t=await fetch(new URL(e,this.baseUrl).href,{method:`GET`,headers:this.buildHeaders()});if(!t.ok)throw Error(`WebDAV GET failed with status ${t.status}`);return await t.blob()}async putFile(e,t){let n;n=t instanceof ReadableStream?await new Response(t).blob():typeof t==`string`?new Blob([t]):t;let r=await fetch(new URL(e,this.baseUrl).href,{method:`PUT`,headers:this.buildHeaders({"Content-Type":`application/octet-stream`}),body:n});if(!r.ok)throw Error(`WebDAV PUT failed with status ${r.status}`)}async deleteResource(e){let t=await fetch(new URL(e,this.baseUrl).href,{method:`DELETE`,headers:this.buildHeaders()});if(!t.ok)throw Error(`WebDAV DELETE failed with status ${t.status}`)}async createDirectory(e){let t=await fetch(new URL(e,this.baseUrl).href,{method:`MKCOL`,headers:this.buildHeaders()});if(!t.ok)throw Error(`WebDAV MKCOL failed with status ${t.status}`)}async moveResource(e,t){let n=new URL(t,this.baseUrl).href,r=await fetch(new URL(e,this.baseUrl).href,{method:`MOVE`,headers:this.buildHeaders({Destination:n,Overwrite:`F`})});if(!r.ok)throw Error(`WebDAV MOVE failed with status ${r.status}`)}async copyResource(e,t){let n=new URL(t,this.baseUrl).href,r=await fetch(new URL(e,this.baseUrl).href,{method:`COPY`,headers:this.buildHeaders({Destination:n,Overwrite:`F`})});if(!r.ok)throw Error(`WebDAV COPY failed with status ${r.status}`)}buildHeaders(e){let t={...e??{}};return this.authHeader&&(t.Authorization=this.authHeader),t}parseMultiStatus(e){let t=new DOMParser().parseFromString(e,`text/xml`).querySelectorAll(`response`),n=[];return t.forEach(e=>{let t=e.querySelector(`href`)?.textContent||``,r=new URL(t,this.baseUrl).href,i=e.querySelector(`displayname`)?.textContent||``,a=!!e.querySelector(`resourcetype`)?.querySelector(`collection`),o=e.querySelector(`getcontenttype`)?.textContent||void 0,s=e.querySelector(`getcontentlength`)?.textContent,c=s?parseInt(s):void 0,l=e.querySelector(`getlastmodified`)?.textContent,u=l?new Date(l):void 0,d=e.querySelector(`getetag`)?.textContent||void 0;n.push({href:r,displayName:i||r.split(`/`).filter(Boolean).pop()||``,isDirectory:a,contentType:o,contentLength:c,lastModified:u,etag:d})}),n}getBaseUrl(){return this.baseUrl}},g=class extends n{constructor(e,t,n){super(),this.client=e,this.resource=t,this.parent=n}getName(){return this.resource.displayName}getParent(){return this.parent}async getContents(e){let t=await this.client.getFile(this.resource.href);return!e||e?.contentType===i.TEXT?await t.text():e?.blob?t:e?.uri?URL.createObjectURL(t):await t.arrayBuffer()}async saveContents(t,n){await this.client.putFile(this.resource.href,t),e(r,o.getWorkspaceSync()??this.getWorkspace())}async size(){return this.resource.contentLength??null}async delete(){await this.client.deleteResource(this.resource.href),e(r,o.getWorkspaceSync()??this.getWorkspace())}async copyTo(e){let t=await this.getWorkspace().getResource(e,{create:!0});if(!t)throw Error(`Failed to create target file: ${e}`);let n=await this.getContents({blob:!0});await t.saveContents(n)}async rename(t){if(this.getName()===t)return;let n=this.resource.href.split(`/`);n[n.length-1]=t;let i=n.join(`/`);await this.client.moveResource(this.resource.href,i),this.resource.href=i,this.resource.displayName=t,e(r,o.getWorkspaceSync()??this.getWorkspace())}},_=class t extends a{constructor(e,t,n,r){if(super(),this.client=e,this.resource=t,this.parent=n,this.connectionInfo=r,!n){let e=r?.name?.trim();this.rootFolderDisplayName=e&&e.length>0?e:v(r?.url??t.href)}}getName(){return this.rootFolderDisplayName===void 0?this.resource.displayName:this.rootFolderDisplayName}getParent(){return this.parent}async listChildren(e=!1){if(e||!this.children){let e=await this.client.propfind(this.resource.href,1);this.children=new Map;for(let n=1;n<e.length;n++){let r=e[n],i=r.isDirectory?new t(this.client,r,this):new g(this.client,r,this);this.children.set(r.displayName,i)}}return Array.from(this.children.values())}async getResource(n,i){if(!n)throw Error(`No path provided`);let a=n.endsWith(`/`),s=n.split(`/`).filter(e=>e.trim()),c=this,l=!1;for(let e=0;e<s.length;e++){let n=s[e];if(c instanceof t){if(await c.listChildren(),!c.children)return null;let r=c.children.get(n);if(!r&&i?.create){let i=this.buildPath(c.resource.href,n);if(e<s.length-1||a){await this.client.createDirectory(i);let e={href:i,displayName:n,isDirectory:!0};r=new t(this.client,e,c),c.children.set(n,r),l=!0}else{await this.client.putFile(i,``);let e={href:i,displayName:n,isDirectory:!1,contentLength:0};r=new g(this.client,e,c),c.children.set(n,r),l=!0}}if(!r||e===s.length-1&&a&&r instanceof g)return null;c=r}}return l&&e(r,o.getWorkspaceSync()??this.getWorkspace()),c}async delete(t,n=!0){if(!t)return this.getParent()?.delete(this.getName());let i=this.buildPath(this.resource.href,t);await this.client.deleteResource(i),this.children?.delete(t),e(r,o.getWorkspaceSync()??this.getWorkspace())}async copyTo(e){for(let t of await this.listChildren()){let n=[e,t.getName()].join(`/`);await t.copyTo(n)}}async rename(t){let n=String(t??``).trim();if(!n||this.getName()===n)return;if(!this.parent){this.rootFolderDisplayName=n,this.connectionInfo&&={...this.connectionInfo,name:n},await o.updateFolderName(this,n);return}let i=this.resource.href.split(`/`).filter(Boolean);i[i.length-1]=n;let a=`/`+i.join(`/`)+`/`;await this.client.moveResource(this.resource.href,a),this.resource.href=a,this.resource.displayName=n,e(r,o.getWorkspaceSync()??this.getWorkspace())}touch(){e(r,o.getWorkspaceSync()??this.getWorkspace())}buildPath(e,t){return e.endsWith(`/`)?e+t:e+`/`+t}getClient(){return this.client}getConnectionInfo(){return this.connectionInfo?this.connectionInfo:{url:this.client.getBaseUrl()}}};function v(e){try{let t=new URL(e).pathname.split(`/`).filter(Boolean);return t[t.length-1]||`workspace`}catch{return`workspace`}}var y=t(`WebDAV`),b=class extends p{constructor(...e){super(...e),this.url=``,this.username=``,this.password=``,this.connecting=!1,this.showHelp=!1}getResult(){return this}async handleConnect(){if(!this.url)return y.error(`Please provide a URL`),!1;try{new URL(this.url)}catch{return y.error(`Invalid URL format`),!1}this.connecting=!0;try{let e,t=this.url.match(/^(https?:\/\/[^\/]+)\/(?:index\.php\/)?s\/([A-Za-z0-9]+)/);if(t){let n=t[1],r=t[2];e={url:`${n}/public.php/webdav/`,username:r,password:this.password||``}}else e={url:this.url,...this.username&&this.password&&{username:this.username,password:this.password}};return await o.connectWorkspace(e),y.info(`Successfully connected to WebDAV workspace`),this.dispatchEvent(new CustomEvent(`connected`,{bubbles:!0,composed:!0})),!0}catch(e){return e instanceof Error?y.error(`Connection failed: ${e.message}`):y.error(`Failed to connect to WebDAV server`),!1}finally{this.connecting=!1}}toggleHelp(){this.showHelp=!this.showHelp}render(){return f`
            <style>
        ...
        `}};l([m()],b.prototype,`url`,void 0),l([m()],b.prototype,`username`,void 0),l([m()],b.prototype,`password`,void 0),l([m()],b.prototype,`connecting`,void 0),l([m()],b.prototype,`showHelp`,void 0),b=l([d(`docks-webdav-connect`)],b);var x=t(`WebDAV`);c({command:{id:`workspace.connect.webdav`,name:`WebDAV / NextCloud`,description:`Connect to WebDAV servers, Nextcloud shares, ownCloud, and other cloud storage`,parameters:[{name:`url`,type:`string`,description:`WebDAV URL: share link (https://cloud.example.com/s/TOKEN) or direct endpoint`,required:!1},{name:`password`,type:`string`,description:`Password (if required)`,required:!1}]},handler:{execute:async e=>{let t=e.parameters?.url,n=e.parameters?.password;if(!t){await s.open(`webdav-connect-dialog`);return}try{let e,r=t.match(/^(https?:\/\/[^\/]+)\/(?:index\.php\/)?s\/([A-Za-z0-9]+)/);if(r){let t=r[1],i=r[2];e={url:`${t}/public.php/webdav/`,username:i,password:n||``}}else e={url:t,...n&&{username:``,password:n}};await o.connectWorkspace(e),x.info(`Connected to WebDAV workspace`)}catch(e){e instanceof Error?x.error(`Failed to connect: ${e.message}`):x.error(`Failed to connect to WebDAV workspace`)}}},contribution:{target:`filebrowser.connections`,name:`filebrowser.connections.webdav`,label:`WebDAV / NextCloud`,icon:`cloud`}});var S=t(`WebDAVExtension`);async function C(){let e=await o.getFolders(),t=new Set(e.filter(e=>e.type===`webdav`).map(e=>e.name)),n=1;for(;t.has(`webdav${n}`);)n+=1;return`webdav${n}`}o.registerContribution({type:`webdav`,name:`webdav`,canHandle(e){return e&&typeof e==`object`&&`url`in e&&typeof e.url==`string`},async connect(e){let t=typeof e.name==`string`&&e.name.trim().length>0?e.name.trim():await C();return new _(new h(e),{href:e.url,displayName:w(e.url),isDirectory:!0},void 0,{...e,name:t})},async restore(e){if(!(!e||!e.url))try{let t={url:e.url,username:e.username,password:e.password?E(e.password):void 0},n=typeof e.name==`string`&&e.name.trim().length>0?e.name.trim():await C();return new _(new h(t),{href:e.url,displayName:w(e.url),isDirectory:!0},void 0,{...t,name:n})}catch(e){S.error(`Failed to restore WebDAV workspace:`,e);return}},async persist(e){if(e instanceof _){let t=e.getConnectionInfo();return t?{url:t.url,name:e.getName(),...t.username===void 0?{}:{username:t.username},...t.password===void 0?{}:{password:T(t.password)}}:null}return null}}),u.registerContribution(`dialogs`,{label:`Connect to WebDAV / NextCloud`,icon:`cloud`,name:`dialog.webdav.connect`,id:`webdav-connect-dialog`,buttons:[{id:`help`,label:`Show help`,variant:`neutral`},{id:`cancel`,label:`Cancel`,variant:`default`},{id:`connect`,label:`Connect`,variant:`primary`}],component:e=>f`<docks-webdav-connect></docks-webdav-connect>`,onButton:async(e,t,n)=>{let r=t;return e===`help`?(r?.toggleHelp(),!1):e===`connect`?r?.handleConnect?await r.handleConnect()?(n?.close?.(),!0):!1:!0:(e===`cancel`&&n?.close?.(),!0)}});function w(e){try{let t=new URL(e).pathname.split(`/`).filter(Boolean);return t[t.length-1]||`workspace`}catch{return`workspace`}}function T(e){try{let t=new TextEncoder().encode(e),n=``;for(let e of t)n+=String.fromCharCode(e);return btoa(n)}catch{return e}}function E(e){try{let t=atob(e),n=new Uint8Array(t.length);for(let e=0;e<t.length;e++)n[e]=t.charCodeAt(e);return new TextDecoder().decode(n)}catch{return e}}export{h as WebDAVClient,_ as WebDAVDirectoryResource,g as WebDAVFileResource};